...
//     try {
//       const res = await fetch("https://api.vintexc.com/apps/auth/register/validate_reg_otp", requestOptions);
//       const data = await res.json()
...
//     try {
//       const res = await fetch("https://api.vintexc.com/apps/auth/register/register",
//       requestOptions);
...
//               {otpStatus === 'loading' ? 'Loading...' : 'Done'}
//             </div>
//           )}
//         </div>
...
const RegistrationForm = () => {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        setOtpStatus('done');
        // 🚩 DEV bypass: autofill OTP for testing
        if (DEV_BYPASS_OTP) {
  ...
  const handleValidateOtp = async () => {
    // 🚩 DEV bypass: accept 000000 without backend
  ...
  const registerUser = async (e) => {
    ...
    const raw = JSON.stringify({
      ...
      username: username,
      password: password,
      otp: "000000" // 🚩 DEV bypass fixed OTP
    });
