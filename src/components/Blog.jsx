...
const Blog = () => {
  ...
  const fetchNews = async () => {
    ...
    try {
      const res = await fetch('https://api.vintexc.com/apps/trade/get_news', requestOptions);
      const data = await res.json();
      return data;
    } catch (error) {
      console.error('Error fetching news:', error);
      ...
      <div className="blog-container">
        ...
        <div className="blog-grid">
          {loading
            ...
            : visibleNews.map((item, index) => (
                <div
                  className={`blog-box ${expandedIndex === index ? 'expanded' : ''}`}
                  key={index}
                >
                  <h2>
                    {expandedIndex === index
                  ...
                  <img src={item.image} loading="lazy" alt="Blog" />
                  <button id="learn-more-btn" onClick={() => toggleExpand(index)}>
                    {expandedIndex === index ? 'Show Less' : 'Learn More'}
                  </button>
                </div>
          ...
          <button
            onClick={() =>
              setCurrentIndex((prev) =>
                news && prev + 4 < news.length ? prev + 4 : prev
              )
            }
            disabled={news && currentIndex + 4 >= news.length}
          >
